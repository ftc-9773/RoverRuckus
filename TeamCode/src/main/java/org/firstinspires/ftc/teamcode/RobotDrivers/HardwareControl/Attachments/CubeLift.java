...
public class CubeLift implements Attachment {
    ...
    public CubeLift(HardwareMap hwmap, boolean initLift){
        ...
        liftScorePos = json.getInt ("liftScorePos",1480);
        minHookOpenPosition = json.getInt ("minHookOpenPosition",3500);

        liftUpTol = json.getInt("liftUpPos", 350);

        minLiftPosition = json.getInt("minLiftPosition", -20);
        maxLiftPosition =json.getInt("maxLiftPosition",1530);
    ...
    public void update() {
        ...
        if(openHook && getLiftPos() > minHookOpenPosition){
            openLatch();
        } else{
            closeLatch();
        }
    ...
    public void closeLatch(){
     ...
     void openLatch()  { hookServo.setPosition(hookOpenPos);}
    ...
    public void writePositionsToTelem(Telemetry telem, boolean stop){
        telem.addData("leftLiftMotorRawPos", leftLiftMotor.getCurrentPosition());
        telem.addData("rightLiftMotorRawPos", rightLiftMotor.getCurrentPosition());
        telem.addData("lift curr. pos scaled", getLiftPos());
